...
class RetrieveCriteria extends PersistentCriteria
{
    ...
    public function joinSql($command) {
        $join = preg_replace("/(?i) inner /", " inner ", $command);
    ...
    public function join($c1, $c2 = '', $condition = '', $joinType = 'INNER')
    {
        if (func_num_args() > 1) {
            $this->addMultipleClasses([$c1, $c2]);
            $this->joins[] = array($c1, $c2, $condition, $joinType);
            return $this;
        } else {
            return $this->joinSql($c1);
        }
    }
    ...
    private function addMultipleClasses(array $classes)
    {
        foreach ($classes as $class) {
            list($className, $alias) = explode(' ', $class);
            $this->addClass(trim($className), trim($alias));
        }
    }

    public function inner($joinClass, $condition) {
        $c = $this->getClassMap()->getName();
        $this->addMultipleClasses([$c, $joinClass]);
        $this->joins[] = [$c, $joinClass, $condition, 'inner'];
        return $this;
    }
    ...
    public function whereParam($command, $parameters)
    {
        $this->where($command);
    ...
    public function asQuery($parameters = null)
    {
        ...
        $query = PersistentManager::processCriteriaAsQuery($this, $parameters);
    ...
    {
        return PersistentManager::processCriteriaAsObjectArray($this, $parameters);
    }
