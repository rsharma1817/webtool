...
class PersistentCriteria extends BaseCriteria
{
    ...
    protected $alias = '';
    protected $maps = []; // array of classMaps
    protected $tableCriteria = [];
    ...
    public function addClass($className, $alias = '', $classMap = NULL)
    {
        ...
        if ((!isset($this->classes[$fullClassName])) || ($alias != '')) {
            $this->classes[$fullClassName] = [$fullClassName, $alias];
        }
        if (!$classMap) {
            $classMap = PersistentManager::getClassMap($fullClassName);
        }
        if ($alias != '') {
    ...
    public function setAlias($alias, $class = NULL)
    {
        if ($class == NULL) {
            $class = $this->classMap;
            $this->alias = $alias;
            $this->classes[$class->getName()] = [$class->getName(), $alias];
        }
    ...
    public function getAssociationsJoin()
    {
        ...
        do {
            for ($i = 0; $i < $classMap->getReferenceSize(); $i++) {
                ...
                $join[] = [$tableName1, $tableName2, $condition, 'INNER'];
            }
            $classMap = $classMap->getSuperClassMap();
        } while ($classMap != NULL);
    ...
    public function getForcedJoin()
    {
        ...
        if (count($this->joins)) {
            foreach ($this->joins as $forcedJoin) {
                ...
                $join[] = [$class1, $class2, $condition, $forcedJoin[3]];
            }
        }
    ...
    public function tableCriteria($criteria, $alias)
    {
        ...
        $this->tableCriteria[] = [$sql->select()->getCommand(), $alias];
        return $this;
    }

    public function joinCriteria($criteria, $condition, $joinType = 'INNER')
    {
        $this->joins[] = [$this->classMap->getName(), $criteria, $condition, $joinType];
    }
    ...
    public function getCriteria($op1, $operator = '', $op2 = NULL)
    {
        ...
        return BaseCriteria::getCondition($operand1, $operator, $operand2);
    }
    ...
    {
        return PersistentManager::processCriteriaAsQuery($this, $parameters);
    }

    public function retrieveAsCursor($parameters = null)
    {
        return PersistentManager::processCriteriaAsCursor($this, $parameters);
    }

    public function retrieveAsProxyQuery($parameters = null)
    {
        return PersistentManager::processCriteriaAsProxyQuery($this, $parameters);
    }

    public function retrieveAsProxyCursor($parameters = null)
    {
        return PersistentManager::processCriteriaAsProxyCursor($this, $parameters);
    }
