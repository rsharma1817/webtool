...
class MController
{
    ...
    */
    ...
    /**
     * Transfere o array de parametros para o objeto data.
     ...
     */
    private function getContent($controller, $view)
    {
        $app = $this->getApplication();
        $module = $this->getModule();
        $base = Manager::getAppPath('', $module, $app);
        $path = '/views/' . $controller . '/' . $view;
        $extensions = ['.xml', '.blade.php', '.vue', '.php', '.html', '.js', '.wiki'];
        $content = '';
        foreach ($extensions as $extension) {
            $fileName = $base . $path . $extension;
            if (file_exists($fileName)) {
                mtrace('MController::getContent ' . $fileName);
                $this->data->_extension = str_replace('.','', $extension);
                $content = $this->renderView($controller, $fileName);
                break;
            } /* else {
     ...
     */
    private function renderContent($viewName = '', $parameters = [])
    {
        ...
        return $this->getContent($controller, $view);
    }
     ...
     * @return string Conteúdo da View.
     */
    private function renderAppView($app, $module, $controller, $viewFile)
    {
        $view = Manager::getView($app, $module, $controller, $viewFile);
        // os parametros são passados para a view através do objeto Data
        return $view->process($this);
    }
     ...
     * @return string Conteúdo da View.
     */
    private function renderView($controller, $viewFile)
    {
        return $this->renderAppView($this->application, $this->module, $controller, $viewFile);
    }
     ...
     */
    public function renderTemplate($templateName, $parameters = [])
    {
        $controller = strtolower($this->name);
     ...
     * @param array $parameters
     */
    public function renderPartial($viewName = '', $parameters = [])
    {
        if (($view = $viewName) == '') {
            $view = $this->action;
        }
     ...
     * @param array $parameters Objeto Data.
     */
    public function renderWindow($viewName = '', $parameters = [])
    {
     ...
     * @param array $parameters Objeto data.
     */
    public function renderFlush($viewName = '', $parameters = [])
    {
