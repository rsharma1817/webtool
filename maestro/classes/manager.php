...
class Manager
{
    ...
    /*
     * Array de configurações conf/_conf.php.
     */
    ...
    public $appPath;
    /*
     * Path do arquivo de configuração do Framework (_conf.php).
     */
    public $confPath;
    /*
    ...
    public $coreAppsPath;
    /*
     * Path do arquivo de configuração em uso (default: _conf.php)
     */
    public $configFile;
    /*
...
//    public $application;
    ...
    {
        $varPath = self::$instance->getOptions('varPath') . DIRECTORY_SEPARATOR . self::$instance->app;
        ...
        self::$instance->setConf('logs.path', $varPath . '/log');
        self::$instance->setLog(self::$instance->app);
        Manager::setSession(new MSession(self::$instance->app));
        Manager::getSession()->init();
    ...
    /**
     * Carrega configurações a partir de um arquivo _conf.php.
     * @param string $configFile
     */
    ...
    public static function getLogin()
    {
        ...
        if (!$login) {
            $login = self::$instance->getAuth()->getLogin();
    ...
    public static function getCache()
    {
        if (!self::$instance->cache) {
            $driver = self::$instance->conf['cache']['type'] ?: 'apcu';
            $cacheObj = new MCacheFast($driver);
            self::$instance->cache = $cacheObj->getCache();
        }
    ...
    public static function getFileURL($file = '', $absolute = false)
    {
        return self::$instance->getBaseURL($absolute) . ($file ? '/' . $file : '');
    }
    ...
    public static function getDownloadURL($controller = '', $file = '', $inline = false, $absolute = true)
    {
        ...
        return self::$instance->getBaseURL($absolute) . '/index.php/core/download' . $controller . '/' . ($inline ? 'inline' : 'save') . '/' . $file;
    }
    ...
    public static function getThemeURL($file = '')
    {
        ...
        // }
        //return $url;
        return self::$instance->getAbsoluteURL("themes/{$theme}/{$file}");
    }
    ...
    {
        $conf = $conf ?: self::$instance->getOptions('db');
    ...
    public static function setDatabase($db, $conf)
    {
