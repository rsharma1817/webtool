...
class MContext
{
    ...
    public function defineContext()
    {
        ...
        if ($app != '') {
            ...
            //$pathUntilNow = Manager::getAbsolutePath("apps/{$this->app}");
            $pathUntilNow = Manager::getHome();
            // checking for module
            $part = array_shift($pathParts);
            $testPath = $pathUntilNow . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . $part;
            if (file_exists($testPath)) {
                $this->module = $part;
                $part = array_shift($pathParts);
                $pathUntilNow = $testPath;
            } else {
                $this->module = '';
            }
            // checking for controller
            $testPath1 = $pathUntilNow . DIRECTORY_SEPARATOR . 'controllers' . DIRECTORY_SEPARATOR . $part . 'Controller.php';
            $testPath2 = $pathUntilNow . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'controllers' . DIRECTORY_SEPARATOR . $part . 'Controller.php';
            if (file_exists($testPath1) || file_exists($testPath2)) {
                $controller = $part;
                $part = array_shift($pathParts);
            } else {
                // checking for service
                $testPath1 = $pathUntilNow . DIRECTORY_SEPARATOR . 'services' . DIRECTORY_SEPARATOR . $part . 'Service.php';
                $testPath2 = $pathUntilNow . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'services' . DIRECTORY_SEPARATOR . $part . 'Service.php';
                if (file_exists($testPath1) || file_exists($testPath2)) {
                    $service = $part;
                    $part = array_shift($pathParts);
                } else {
                    // checking for component
                    $testPath1 = $pathUntilNow . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . $part . '.php';
                    $testPath2 = $pathUntilNow . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . $part . '.php';
                    if (file_exists($testPath1) || file_exists($testPath2)) {
                        $component = $part;
                        $part = array_shift($pathParts);
                    } else {
                        // checking for DDD services
                        $testPath = $pathUntilNow . DIRECTORY_SEPARATOR . $part;
                        $nextPart = array_shift($pathParts);
                        if (is_dir($testPath)) {
                            $api = $part;
                            $service = $nextPart;
                            $apiPart = $service;
                            $testPath .= DIRECTORY_SEPARATOR . $service;
                            //mdump('>>>>'.$dir);
                            $nextPart = array_shift($pathParts);
                            if (is_dir($testPath)) {
                                $system = $service;
