...
//        return self::$instance->getBaseURL($absolute) . (self::$instance->java ? '' : '/apps') . '/' . $app . '/public' . ($file ? '/' . $file : '');
    ...
    */
    ...
    public static function createFileMap(Event $event)
    {
        $vendorDir = $event->getComposer()->getConfig()->get('vendor-dir');
        var_dump($vendorDir);
        $baseDir = dirname($vendorDir) . DIRECTORY_SEPARATOR . 'apps' . DIRECTORY_SEPARATOR . 'webtool';
        ...
        if (file_exists($baseDirSrc)) {
            ...
            $newMap .= "\$baseDir = dirname(dirname(__FILE__)) . '" . DIRECTORY_SEPARATOR . 'apps' . DIRECTORY_SEPARATOR . 'webtool' . "';\n\n";
        }
        $newMap .= "return array(\n";
