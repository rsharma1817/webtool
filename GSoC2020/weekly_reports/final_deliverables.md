...
Since we only project a frame element label when they span the entire constituent, constituent-to-word projection reduces the number of frame elements projected. Specifically, if only a single word in the two-word constituent is labeled with a frame element label, the label is not projected to the target sentence. Surprisingly, this characteristic results in a better projection because the Portuguese sentences and German sentences are under-labeled. For example, for the EN-PT sentence pair "We know three things about intelligence ." and "Sabemos três coisas sobre inteligência ."[(We) know three things about intelligence .] that shares the same semantic frame *Topic*, the English phrase "three things" is labeled with the role *Text* but not the Portuguese phrase "três coisas". In essence, it is uncertain whether we can generalize the success of the constituent-to-word projection that is contingent upon the annotation discrepancies.
