...
Assuming that semantic frame can be successfully projected (i.e., there's no divergence), can the frame elements can be projected as well? When the sentence pairs are annotated with the same semantic frame, only 32.7% of the annotations for EN-PT pairs and 43.8% for EN-DE pairs share the same set of frame element labels, including core and non-core frame elements. The figures are the upper bound for the exact match ratio if we treat the annotation projection as a sequence labeling task using all the labels from the annotated parallel sentences.
