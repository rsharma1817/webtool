...
class MBaseControl extends MBase
{
    ...
    function onGenerate()
    {
        ...
        $this->getPainter()->generateEvents($this);
    }
    ...
    public function fetch($template = '', $args = []) {
        return $this->getPainter()->fetch($template ?: $this->className, $this, $args);
    }

    public function render($args = [], $template = '') {
        if ($this->hasItems()) {
            $inner = $this->getPainter()->generateToString($this->controls);
        } elseif ($this->property->cdata) {
            $inner = $this->property->cdata;
        } else {
            $inner = $this->inner;
        }
        $params = array_merge(['inner' => $inner], $args);
