...
class MDialog extends MControl
{
    ...
    public function render()
    {
        ...
        $this->setId($this->id);
        $this->plugin = 'dialog';
        $this->options->modal = ($this->property->modal === false) ? false : true;
        $this->options->doSize = true;
        if ($this->property->file) {
            $dialog = $this->getControlsFromXML($this->property->file);
            $this->addControls($dialog);
        }
        $onClose = "function() {" . $this->property->onClose . " $('#{$this->property->id}').dialog('destroy', true);}";
        $this->options->onClose = (object) $onClose;
        $state = ($this->property->state == "open") ? 'false' : 'true';
        $onLoad = <<<EOT
$('#{$this->property->id}').dialog({closed: {$state}});
$('#{$this->property->id}').dialog('resize',{width:'auto',height:'auto'});
EOT;
        $this->page->onLoad($onLoad);
        $tools = $this->generateTools();
        */
        $onLoad = <<<EOT
$('#{$this->property->id}')..modal('show');
EOT;
        $this->page->onLoad($onLoad);
